...
export default function SelectPlan(props) {
    const timePeriodStyles = {"color": "#001126", "font-weight": "700"};
    ...
    const [arcadeStyles, setArcadeStyles] = useState({});
    const [advancedStyles, setAdvancedStyles] = useState({});
    const [proStyles, setProStyles] = useState({});
    const [timePeriodBool, setTimePeriodBool] = useState(false);
    function changeTimePeriod(state){
        if(state){
            setYearlyStyles(timePeriodStyles);
            setMonthlyStyles({"color": "#676767", "font-weight": "600"});
            setTimePeriodBool(true);
        }
        else {
            setMonthlyStyles(timePeriodStyles);
            setYearlyStyles({"color": "#676767", "font-weight": "600"});
            setTimePeriodBool(false);
        }
    }

    function handleArcade(){
        setArcadeStyles(planStyles);
        setAdvancedStyles({});
        setProStyles({});

        props.setPlan("Arcade");
        props.setTotal("$9");
        if(!timePeriodBool)props.setGrandTotal("$108");
        else props.setGrandTotal("$9");
    }
    function handleAdvanced(){
        setArcadeStyles({});
        setAdvancedStyles(planStyles);
        setProStyles({});

        props.setPlan("Advanced");
        props.setTotal("$12");
        if(!timePeriodBool)props.setGrandTotal("$144");
        else props.setGrandTotal("$12");
    }
    function handlePro(){
        setArcadeStyles({});
        setAdvancedStyles({});
        setProStyles(planStyles);

        props.setPlan("Pro");
        props.setTotal("$15");
        if(!timePeriodBool)props.setGrandTotal("$180");
        else props.setGrandTotal("$15");
    }
    return (
        <div className={styles.form}>
            ...
            <div className={styles.gridContainer}>
                <div style={arcadeStyles} className={styles.arcade} onClick={handleArcade}>
                ...
                <div style={advancedStyles} className={styles.advanced} onClick={handleAdvanced}>
                ...
                <div style={proStyles} className={styles.pro} onClick={handlePro}>
            ...
            <div className={styles.options}>
                <p style={monthlyStyles}>Monthly</p>
                <p style={yearlyStyles}>Yearly</p> 
                <Toggle toggled={false} onClick={changeTimePeriod}/>
            </div>
        </div>
    );
}